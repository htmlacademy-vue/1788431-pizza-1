...
export default {
  ...
  computed: {
    filteredTasks() {
      const filtersAreEmpty = Object.values(this.filters).every(
        (value) => !value.length
      );
      if (filtersAreEmpty) {
      ...
      const usersFilter = (task) =>
        this.filters.users.some((userId) => userId === task.userId);

      const statusesFilter = (task) =>
        this.filters.statuses.some(
      ...
      return this.tasks.filter((task) => {
        ...
        return Object.entries(result).every(
          ([key, callback]) => !this.filters[key].length || callback(task)
        );
      });
    },
  },
